...
def get_main_keyboard():
    ...
    markup.add(types.KeyboardButton('أسعار النقاط'))
    markup.add(types.KeyboardButton('الشحن عبر التحويل'), types.KeyboardButton('شحن عبر الكارت'))
    return markup
...
def start(message):
    ...
    if not db['users'][user_id]['phone']:
        ...
        bot.send_message(message.chat.id, '👋 أهلاً بك مجدداً. اختر من القائمة أدناه:', reply_markup=get_main_keyboard())
...
def handle_text(message):
    ...
    if text == 'أسعار النقاط':
        bot.send_message(message.chat.id, db['price_message'])
    
    elif text == 'الشحن عبر التحويل':
        user_states[user_id] = {'step': 'transfer_amount'}
        bot.send_message(message.chat.id, '💰 اكتب المبلغ المراد تحويله ($):')
    
    elif text == 'شحن عبر الكارت':
